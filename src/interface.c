...
int draw_selection_space(ParamInterface* gp, Plugin* plugins, int nb_plugin, Space* W){
  ...
  for (i=0 ; i<nb_plugin ; i++){
	...
	MLV_draw_text(abs_current+10, ord_current, plugins[i].name, MLV_rgba(255,255,255,255));
      else
  ...
  if (((x > 70) && (x < 670)) && ((y > 110) && (y < 750))){
    ...
    if (x > 470)
      selected_plugin += 16;
    if (selected_plugin < nb_plugin){
      if (nb_starship < 12 && !(plugins[selected_plugin].selected)){
	plugins[selected_plugin].selected = 1;
	new_starship = create_starship(W->starships, 0);
	new_starship->plugin = &(plugins[selected_plugin]);
	plugins[selected_plugin].starship = new_starship;
	current = W->starships;
	if (current == NULL){
